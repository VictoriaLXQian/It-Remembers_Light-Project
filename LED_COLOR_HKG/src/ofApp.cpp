...
//    bool nc = jsonNc.open(urlNc);
    ...
    //
    int baud = 57600;
    ard.connect("/dev/cu.usbmodem146401",baud);
...
//        usa = false;
    ...
    //        milan = false;
    if (hongkongTime > hongkongSunrise && hongkongTime < hongkongSunset) {
        hongkong = true;
    }else if (hongkongTime < hongkongSunrise||hongkongTime > hongkongSunset)
        hongkong = false;
...
void ofApp::updateArduino(){
    ...
    if (bSetupArduino){
        ...
        //            ofDrawBitmapString(msg, 200, 400);
        if (hongkong == true) {
            ard.sendPwm(11, 236); //R
            ard.sendPwm(10, 192); //G
            ard.sendPwm(6, 160); //B
            msg = "HK Day";
            ofDrawBitmapString(msg, 200, 400);
        }else if (hongkong == false) {
            ard.sendPwm(11, 0);
            ard.sendPwm(10, 0);
            ard.sendPwm(6, 0);
        };
